...
func performanceCommand() {
	...
	var hash [][32]byte
	var err error
	blockIterations := 10000
	stateIterations := 1000

	fmt.Printf("## mainnet preset / BeaconBlock decode + encode + hash (%d times)\n", blockIterations)
	dur, hash, err = test_block_fastssz(block_mainnet, blockIterations)
	print_test_result("fastssz only", dur, hash, err)
	dur, hash, err = test_block_dynssz(dynssz_only_mainnet, block_mainnet, blockIterations)
	print_test_result("dynssz only", dur, hash, err)
	dur, hash, err = test_block_dynssz(dynssz_hybrid_mainnet, block_mainnet, blockIterations)
	print_test_result("dynssz + fastssz", dur, hash, err)
	dur, hash, err = test_block_dynssz_codegen(dynssz_hybrid_mainnet, block_mainnet, blockIterations)
	print_test_result("dynssz + codegen", dur, hash, err)
	fmt.Printf("\n")

	fmt.Printf("## mainnet preset / BeaconState decode + encode + hash (%d times)\n", stateIterations)
	dur, hash, err = test_state_fastssz(state_mainnet, stateIterations)
	print_test_result("fastssz only", dur, hash, err)
	dur, hash, err = test_state_dynssz(dynssz_only_mainnet, state_mainnet, stateIterations)
	print_test_result("dynssz only", dur, hash, err)
	dur, hash, err = test_state_dynssz(dynssz_hybrid_mainnet, state_mainnet, stateIterations)
	print_test_result("dynssz + fastssz", dur, hash, err)
	dur, hash, err = test_state_dynssz_codegen(dynssz_hybrid_mainnet, state_mainnet, stateIterations)
	print_test_result("dynssz + codegen", dur, hash, err)
	fmt.Printf("\n")

	fmt.Printf("## minimal preset / BeaconBlock decode + encode + hash (%d times)\n", blockIterations)
	dur, hash, err = test_block_fastssz(block_minimal, blockIterations)
	print_test_result("fastssz only", dur, hash, err)
	dur, hash, err = test_block_dynssz(dynssz_only_minimal, block_minimal, blockIterations)
	print_test_result("dynssz only", dur, hash, err)
	dur, hash, err = test_block_dynssz(dynssz_hybrid_minimal, block_minimal, blockIterations)
	print_test_result("dynssz + fastssz", dur, hash, err)
	dur, hash, err = test_block_dynssz_codegen(dynssz_hybrid_minimal, block_minimal, blockIterations)
	print_test_result("dynssz + codegen", dur, hash, err)
	fmt.Printf("\n")

	fmt.Printf("## minimal preset / BeaconState decode + encode + hash (%d times)\n", stateIterations)
	dur, hash, err = test_state_fastssz(state_minimal, stateIterations)
	print_test_result("fastssz only", dur, hash, err)
	dur, hash, err = test_state_dynssz(dynssz_only_minimal, state_minimal, stateIterations)
	print_test_result("dynssz only", dur, hash, err)
	dur, hash, err = test_state_dynssz(dynssz_hybrid_minimal, state_minimal, stateIterations)
	print_test_result("dynssz + fastssz", dur, hash, err)
	dur, hash, err = test_state_dynssz_codegen(dynssz_hybrid_minimal, state_minimal, stateIterations)
	print_test_result("dynssz + codegen", dur, hash, err)
	fmt.Printf("\n")
