...
if [ -n "${FORK}" ]; then
    ...
    go test -v -timeout=30m -coverprofile=spec-coverage.out -covermode=atomic -coverpkg=github.com/pk910/dynamic-ssz -run="TestConsensusSpec${FORK_CAPITALIZED}" ./...
else
    echo "Running all tests for preset: ${PRESET}"
    go test -v -timeout=30m -coverprofile=spec-coverage.out -covermode=atomic -coverpkg=github.com/pk910/dynamic-ssz -run="TestConsensusSpec" ./...
fi
