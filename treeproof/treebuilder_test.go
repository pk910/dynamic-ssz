...
func TestWrapperCommitAliases(t *testing.T) {
	build := func(fn func(w *treeBuilder)) []byte {
		w := newTreeBuilder()
		w.PutUint64(1)
		w.PutUint64(2)
		w.PutUint64(3)
	...
	if !bytes.Equal(build(func(w *treeBuilder) { w.Commit(0) }),
		build(func(w *treeBuilder) { w.Merkleize(0) })) {
		t.Error("Commit should match Merkleize")
	}
	if !bytes.Equal(build(func(w *treeBuilder) { w.CommitWithMixin(0, 3, 8) }),
		build(func(w *treeBuilder) { w.MerkleizeWithMixin(0, 3, 8) })) {
		t.Error("CommitWithMixin should match MerkleizeWithMixin")
	}
	if !bytes.Equal(build(func(w *treeBuilder) { w.CommitProgressive(0) }),
		build(func(w *treeBuilder) { w.MerkleizeProgressive(0) })) {
		t.Error("CommitProgressive should match MerkleizeProgressive")
	}
	if !bytes.Equal(build(func(w *treeBuilder) { w.CommitProgressiveWithMixin(0, 3) }),
		build(func(w *treeBuilder) { w.MerkleizeProgressiveWithMixin(0, 3) })) {
		t.Error("CommitProgressiveWithMixin should match MerkleizeProgressiveWithMixin")
	}
	activeFields := []byte{0x07}
	if !bytes.Equal(build(func(w *treeBuilder) { w.CommitProgressiveWithActiveFields(0, activeFields) }),
		build(func(w *treeBuilder) { w.MerkleizeProgressiveWithActiveFields(0, activeFields) })) {
...
// treeBuilder and Hasher must agree for empty PutBytes/PutProgressiveBitlist input:
...
// treeBuilder.PutBitlist with maxSize 0 must behave like Hasher.PutBitlist (a
...
func TestWrapperPutBitlistZeroMaxMatchesHasher(t *testing.T) {
	...
	func() {
		defer func() {
			if r := recover(); r != nil {
				t.Fatalf("treeBuilder.PutBitlist(_, 0) panicked: %v", r)
			}
		}()
		w := newTreeBuilder()
...
// treeBuilder.MerkleizeWithMixin with a limit below the chunk count must clamp
...
func TestWrapperMixinLimitBelowChunksMatchesHasher(t *testing.T) {
	...
	func() {
		defer func() {
			if r := recover(); r != nil {
				t.Fatalf("treeBuilder.MerkleizeWithMixin(limit < count) panicked: %v", r)
			}
		}()
		w := newTreeBuilder()
	...
	func() {
		defer func() {
			if r := recover(); r != nil {
				t.Fatalf("treeBuilder.PutUint64Array(cap < len) panicked: %v", r)
			}
		}()
		w2 := newTreeBuilder()
...
// treeBuilder.Merkleize* must clamp an out-of-range scope index like Hasher does,
// instead of panicking with a raw slice-bounds error.
func TestWrapperMerkleizeIndexClamped(t *testing.T) {
	cases := []func(w *treeBuilder){
		func(w *treeBuilder) { w.Merkleize(999) },
		func(w *treeBuilder) { w.Merkleize(-1) },
		func(w *treeBuilder) { w.MerkleizeWithMixin(-1, 1, 4) },
		func(w *treeBuilder) { w.MerkleizeProgressive(999) },
		func(w *treeBuilder) { w.MerkleizeProgressiveWithMixin(-1, 1) },
	}
...
// --- treeBuilder.Collapse: no-op coverage ---
...
// --- treeBuilder.Commit: panic on tree build error ---
...
// --- treeBuilder.CommitWithMixin: panic on tree build error ---
...
// --- treeBuilder.CommitProgressive: panic on tree build error ---
...
// --- treeBuilder.CommitProgressiveWithMixin: panic on tree build error ---
...
// --- treeBuilder.CommitProgressiveWithActiveFields: panic on tree build error ---
...
// https://github.com/pk910/dynamic-ssz/issues/191: the treeBuilder must produce
// the same root as hasher.Hasher when a container interleaves Append*-buffered
// fields with Put* fields or child scopes. The treeBuilder used to flush buffered
...
// treeBuilder.PutBytes must not write zero padding into the caller's backing
...
func TestWrapperPutBytesDoesNotMutateCallerMemory(t *testing.T) {
	...
	if !bytes.Equal(backing, before) {
		t.Fatalf("treeBuilder.PutBytes mutated caller memory:\n before: %x\n after:  %x", before, backing)
	}
	...
	if !bytes.Equal(backing, before) {
		t.Fatalf("treeBuilder.PutBytes (multi-chunk) mutated caller memory")
	}
...
// hash. The treeBuilder is documented as a drop-in HashWalker producing the same
