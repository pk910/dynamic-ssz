...
var _ sszutils.HashWalker = (*treeBuilder)(nil)

// treeBuilder implements the sszutils.HashWalker interface to construct a complete
...
// newTreeBuilder creates a full-tree walker with an empty node list and
...
// --- treeBuilder implements the HashWalker interface ---
...
func (w *treeBuilder) WithTemp(fn func(tmp []byte) []byte) {
	w.tmp = fn(w.tmp)
}
...
func (w *treeBuilder) Index() int {
	w.flushBuffer()
	return len(w.nodes)
}

// CurrentIndex returns the current buffer index (debug only)
func (w *treeBuilder) CurrentIndex() int {
	return len(w.buf)
}
...
func (w *treeBuilder) AppendBytes32(b []byte) {
...
func (w *treeBuilder) Merkleize(indx int) {
	w.flushBuffer()
	w.commit(indx)
}
...
func (w *treeBuilder) MerkleizeWithMixin(indx int, num, limit uint64) {
	w.flushBuffer()
...
func (w *treeBuilder) MerkleizeProgressive(indx int) {
	w.flushBuffer()
...
func (w *treeBuilder) MerkleizeProgressiveWithMixin(indx int, num uint64) {
	w.flushBuffer()
...
func (w *treeBuilder) MerkleizeProgressiveWithActiveFields(indx int, activeFields []byte) {
	w.flushBuffer()
...
func (w *treeBuilder) PutBitlist(bb []byte, maxSize uint64) {
...
// with a length mixin.
func (w *treeBuilder) PutProgressiveBitlist(bb []byte) {
...
// hasher.Hasher.
func (w *treeBuilder) flushBuffer() {
	if len(w.buf) != 0 {
...
func (w *treeBuilder) appendBytesAsNodes(b []byte) {
...
func (w *treeBuilder) PutBool(b bool) {
...
func (w *treeBuilder) PutUint64Array(b []uint64, maxCapacity ...uint64) {
	indx := w.Index()
	for _, i := range b {
		w.AppendUint64(i)
	}
...
func (w *treeBuilder) AddBytes(b []byte) {
	if len(b) == 0 {
...
func (w *treeBuilder) AddNode(n *Node) {
	w.flushBuffer()
...
// hasher.Hasher, which the treeBuilder is interchangeable with).
func (w *treeBuilder) clampIndex(i int) int {
	if i < 0 {
		return 0
	}
	if i > len(w.nodes) {
...
// node list.
func (w *treeBuilder) commit(i int) {
	i = w.clampIndex(i)
...
func (w *treeBuilder) commitWithMixin(i int, num, limit uint64) {
	i = w.clampIndex(i)
...
func (w *treeBuilder) commitProgressive(i int) {
	i = w.clampIndex(i)
...
func (w *treeBuilder) commitProgressiveWithMixin(i int, num uint64) {
	i = w.clampIndex(i)
	// create progressive tree from nodes
...
func (w *treeBuilder) commitProgressiveWithActiveFields(i int, activeFields []byte) {
	i = w.clampIndex(i)
	// create progressive tree from nodes
...
func (w *treeBuilder) addEmpty() {
	w.AddNode(EmptyLeaf())
}

// The methods below preserve the exported v1.x treeBuilder API (Commit*/AddEmpty)
...
func (w *treeBuilder) CommitProgressive(i int) { w.MerkleizeProgressive(i) }
...
func (w *treeBuilder) CommitProgressiveWithMixin(i, num int) {
...
func (w *treeBuilder) AddEmpty() { w.addEmpty() }
...
func (w *treeBuilder) HashRoot() ([32]byte, error) {
