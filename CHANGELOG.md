...
### Added
- **Unknown-size stream decoding** — `UnmarshalSSZReader` with `size < 0` now decodes
  straight from the reader to EOF instead of buffering the whole payload first. SSZ is
  self-delimiting for every region except the trailing one, so the missing length only
  affects the last dynamic child at each nesting level; supported in both the reflection
  and codegen paths.
- `WithMaxStreamSize` bounds an unknown-size decode (512 MiB by default). The bound cannot
  be disabled: it is what keeps an endless stream from exhausting memory, and `ssz-max`
  limits are now enforced incrementally while reading rather than after buffering.
- `Decoder` gains `LengthKnown`, `More`, `DecodeRemaining` and `PushOpenLimit`;
  `sszutils.NewUnknownStreamDecoder` constructs a decoder for a payload of unknown length.
  `LengthKnown` reports whether a region's extent is backed by input known to exist, not
  merely whether a limit is set: inside an open region an offset is validated against the
  allowance, so a limit derived from one can declare far more bytes than were delivered and
  must never be used as an allocation size.

### Changed
- Errors from an unknown-size decode are less precise: checks that a known length catches
  up front surface as `ErrUnexpectedEOF` when the read runs out. The accept/reject decision
  is unchanged.

### Fixed
- The `DynamicViewUnmarshaler` reflection path now uses the type's fixed size when it has
  one, matching the fastssz and `DynamicUnmarshaler` paths, instead of always consuming the
  whole remaining region.
- Generated decoders no longer read the trailing-element condition through a name a nested
  list can shadow, which made the push and the pop compare against two different lists.

### Upgrade notes
- Types whose SSZ methods were generated by v1.3.2 or earlier must be **regenerated** to be
  decoded with `size < 0`; until then such a call fails cleanly with `ErrUnexpectedEOF`.
  Passing an explicit size keeps working unchanged.
- `sszutils.Decoder` gained methods. This only affects code that *implements* the interface;
  generated code and other callers are unaffected.
