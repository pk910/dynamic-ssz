...
whole cycle; a later batch reaches an earlier batch's members through the
depth-carrying methods it finds in the package source, so the count continues
across the boundary. A member generated by a later batch cannot see the cycle
through the earlier batch's delegated shell and does not count its own level
when it is the entry point, so the bound is exact within one batch and off by
at most one level across batches. Generation through the Go API from
...
See [SSZ Annotations](ssz-annotations.md#type-annotations-with-sszutilsannotatet-non-struct-types) for full syntax details.
