...
func generateUnmarshal(rootTypeDesc *dynssz.TypeDescriptor, codeBuilder *strings.Builder, typePrinter *TypePrinter, options *CodeGeneratorOptions) error {
	...
	ctxTypeName := fmt.Sprintf("unmarshalCtx_%s", strings.ReplaceAll(typeName, "*", "_"))
	ctx.ctxTypeName = ctxTypeName
	...
	codeBuilder.WriteString(fmt.Sprintf("type %s struct {\n", ctxTypeName))
	if ctx.sizeVarCounter > 0 {
		codeBuilder.WriteString(fmt.Sprintf("\tsize [%v]int\n", ctx.sizeVarCounter))
	}
	if ctx.exprVarCounter > 0 {
		codeBuilder.WriteString(fmt.Sprintf("\texpr [%v]uint64\n", ctx.exprVarCounter))
	}
	if ctx.usedDynSpecs {
		codeBuilder.WriteString("\tds sszutils.DynamicSpecs\n")
	}
	...
	if genStaticFn {
		if !ctx.usedDynSpecs {
			...
			if ctx.usedCtx {
				codeBuilder.WriteString(fmt.Sprintf("\tctx := &%s{}\n", ctxTypeName))
			}
	...
	if genDynamicFn {
		if ctx.usedDynSpecs {
			...
			if ctx.usedCtx {
				codeBuilder.WriteString(fmt.Sprintf("\tctx := &%s{ds: ds}\n", ctxTypeName))
			}
			codeBuilder.WriteString(exprCodeBuf.String())
			codeBuilder.WriteString(sizeCodeBuf.String())
			codeBuilder.WriteString(codeBuf.String())
...
func (ctx *unmarshalContext) resolvePtrVar(varName string) string {
...
func (ctx *unmarshalContext) getStaticSizeVar(desc *dynssz.TypeDescriptor) (string, error) {
	...
	sizeVar := fmt.Sprintf("ctx.size[%d]", ctx.sizeVarCounter)
	ctx.sizeVarCounter++
	ctx.usedCtx = true
	...
	case dynssz.SszVectorType, dynssz.SszBitvectorType, dynssz.SszUint128Type, dynssz.SszUint256Type:
		...
		if desc.ElemDesc.SszTypeFlags&dynssz.SszTypeFlagIsDynamic != 0 {
			...
			if sizeExpression != nil {
				exprVar, err := ctx.getLimitExprVar(desc)
				if err != nil {
					return "", err
				}
				...
				ctx.appendSizeCode(1, "%s = %s * int(%s) // size expression for '%s'\n", sizeVar, itemSizeVar, exprVar, ctx.typePrinter.TypeStringWithoutTracking(desc))
			} else {
...
// getStaticSizeVar generates a variable name for cached static size calculations.
func (ctx *unmarshalContext) getLimitExprVar(desc *dynssz.TypeDescriptor) (string, error) {
	if desc.SszType != dynssz.SszVectorType && desc.SszType != dynssz.SszBitvectorType && desc.SszType != dynssz.SszUint128Type && desc.SszType != dynssz.SszUint256Type {
		return "", fmt.Errorf("limit expression not supported for type: %v", desc.SszType)
	}
	...
	if sizeExpression == nil {
		return fmt.Sprintf("%v", desc.Len), nil
	}

	exprKey := sha256.Sum256([]byte(fmt.Sprintf("%s\n%v|%v", *sizeExpression, desc.Len, desc.SszTypeFlags&dynssz.SszTypeFlagHasBitSize != 0)))
	if exprVar, ok := ctx.exprVarMap[exprKey]; ok {
		return exprVar, nil
	}

	exprVar := fmt.Sprintf("ctx.expr[%d]", ctx.exprVarCounter)
	ctx.exprVarCounter++
	ctx.usedCtx = true
...
func (ctx *unmarshalContext) unmarshalVector(desc *dynssz.TypeDescriptor, varName string, indent int, noBufCheck bool) error {
	...
	if sizeExpression != nil && needExpression {
		...
		if desc.SszTypeFlags&dynssz.SszTypeFlagHasBitSize != 0 {
			bitlimitVar = exprVar
			ctx.appendCode(indent, "limit := int((%s+7)/8)\n", exprVar)
			limitVar = "limit"
		} else {
	...
	if desc.ElemDesc.SszTypeFlags&dynssz.SszTypeFlagIsDynamic == 0 {
		...
		ctx.appendCode(indent, "if err := sszutils.UnmarshalStaticVector(ctx, buf, %s, %s, %s, %s, %s, func(ctx *%s, buf []byte, v *%s) error {\n", ctx.resolvePtrVar(varName), limitVar, fieldSizeVar, isArrayStr, noBufCheckStr, ctx.ctxTypeName, ctx.typePrinter.TypeString(desc.ElemDesc))
		if desc.ElemDesc.GoTypeFlags&dynssz.GoTypeFlagIsPointer != 0 {
			ctx.appendCode(indent+1, "if *v == nil {\n\t*v = new(%s)\n}\n", ctx.typePrinter.InnerTypeString(desc.ElemDesc))
		}
		if err := ctx.unmarshalType(desc.ElemDesc, "(*v)", indent+1, false, true); err != nil {
			return err
		}

		ctx.usedCtx = true

		ctx.appendCode(indent, "\treturn nil\n")
		ctx.appendCode(indent, "}); err != nil {\n\treturn err\n}\n")
	} else {
		// dynamic elements
		isArrayStr := "false"
		if desc.Kind == reflect.Array {
		...
		ctx.appendCode(indent, "if err := sszutils.UnmarshalDynamicVector(ctx, buf, %s, %s, %s, %s, func(ctx *%s, buf []byte, v *%s) error {\n", ctx.resolvePtrVar(varName), limitVar, isArrayStr, noBufCheckStr, ctx.ctxTypeName, ctx.typePrinter.TypeString(desc.ElemDesc))
		if desc.ElemDesc.GoTypeFlags&dynssz.GoTypeFlagIsPointer != 0 {
			ctx.appendCode(indent+1, "if *v == nil {\n\t*v = new(%s)\n}\n", ctx.typePrinter.InnerTypeString(desc.ElemDesc))
		}
		if err := ctx.unmarshalType(desc.ElemDesc, "(*v)", indent+1, false, true); err != nil {
...
func (ctx *unmarshalContext) unmarshalList(desc *dynssz.TypeDescriptor, varName string, indent int) error {
	if desc.ElemDesc.SszTypeFlags&dynssz.SszTypeFlagIsDynamic == 0 {
		...
		fieldTypeStr := ctx.typePrinter.TypeString(desc.ElemDesc)
		isArrayStr := "false"
		if desc.Kind == reflect.Array {
			isArrayStr = "true"
			ctx.appendCode(indent, "val := %s[:]\n", varName)
		...
		ctx.appendCode(indent, "if err := sszutils.UnmarshalStaticList(ctx, buf, %s, %s, %s, func(ctx *%s, buf []byte, v *%s) error {\n", ctx.resolvePtrVar(varName), fieldSizeVar, isArrayStr, ctx.ctxTypeName, fieldTypeStr)
		if desc.ElemDesc.GoTypeFlags&dynssz.GoTypeFlagIsPointer != 0 {
			ctx.appendCode(indent+1, "if *v == nil {\n\t*v = new(%s)\n}\n", ctx.typePrinter.InnerTypeString(desc.ElemDesc))
		}
		if err := ctx.unmarshalType(desc.ElemDesc, "(*v)", indent+1, false, true); err != nil {
		...
		ctx.appendCode(indent, "if err := sszutils.UnmarshalDynamicList(ctx, buf, %s, %s, func(ctx *%s, buf []byte, v *%s) error {\n", ctx.resolvePtrVar(varName), isArrayStr, ctx.ctxTypeName, fieldTypeStr)
		if desc.ElemDesc.GoTypeFlags&dynssz.GoTypeFlagIsPointer != 0 {
			ctx.appendCode(indent+1, "if *v == nil {\n\t*v = new(%s)\n}\n", ctx.typePrinter.InnerTypeString(desc.ElemDesc))
		}
		if err := ctx.unmarshalType(desc.ElemDesc, "(*v)", indent+1, false, true); err != nil {
