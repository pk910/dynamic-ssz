...
func TestBuildCompatibleUnionDescriptor(t *testing.T) {
	...
	t.Run("UnionWithEmptyDescriptor", func(t *testing.T) {
		...
		namedEmpty, ok := instantiated.(*types.Named)
		if !ok {
			t.Fatalf("Expected *types.Named, got %T", instantiated)
		}
		...
		err = parser.buildCompatibleUnionDescriptor(desc, namedEmpty)
		if err == nil {
	...
	t.Run("UnionWithNonStructDescriptor", func(t *testing.T) {
		...
		err = parser.buildCompatibleUnionDescriptor(desc, namedNonStruct)
		if err == nil {
	...
	t.Run("UnionWithTaggedVariants", func(t *testing.T) {
		...
		namedType, ok := instantiated.(*types.Named)
		if !ok {
			t.Fatalf("Failed to cast instantiated type to *types.Named")
		}
		err = parser.buildCompatibleUnionDescriptor(desc, namedType)
		if err != nil {
			t.Fatalf("Failed to build CompatibleUnion descriptor: %v", err)
		}
...
func TestBuildTypeWrapperDescriptor(t *testing.T) {
	...
	t.Run("WrapperWithValidDescriptor", func(t *testing.T) {
		...
		namedWrapper, ok := instantiated.(*types.Named)
		if !ok {
			t.Fatalf("Failed to cast instantiated type to *types.Named")
		}
		err = parser.buildTypeWrapperDescriptor(desc, namedWrapper, nil, nil, nil)
		if err != nil {
	...
	t.Run("WrapperWithNonStructDescriptor", func(t *testing.T) {
		...
		err = parser.buildTypeWrapperDescriptor(desc, namedNonStruct, nil, nil, nil)
		if err == nil {
			t.Error("Expected error for non-struct descriptor")
		}
		if !strings.Contains(err.Error(), "must be a struct") {
	...
	t.Run("WrapperWithWrongFieldCount", func(t *testing.T) {
		...
		err = parser.buildTypeWrapperDescriptor(desc, instantiated.(*types.Named), nil, nil, nil) //nolint:errcheck // error is checked below
		if err == nil {
	...
	t.Run("WrapperWithStaticType", func(t *testing.T) {
		...
		err = parser.buildTypeWrapperDescriptor(desc, instantiated.(*types.Named), nil, nil, nil) //nolint:errcheck // type assertion is safe here
		if err != nil {
			t.Fatalf("Failed to build TypeWrapper descriptor: %v", err)
		}
...
func TestExtendedTypesDisabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := parser.buildTypeDescriptor(tt.goType, typeHint, nil, nil)
			if err == nil {
...
func TestOptionalNonPointer(t *testing.T) {
	...
	_, err := parser.buildTypeDescriptor(types.Typ[types.Uint64], typeHint, nil, nil)
	if err == nil {
...
func TestOptionalWithHints(t *testing.T) {
	...
	desc, err := parser.buildTypeDescriptor(ptrType, typeHint, sizeHint, maxHint)
	if err != nil {
...
func TestCompatibleUnionNonNamed(t *testing.T) {
	...
	_, err := parser.buildTypeDescriptor(structType, typeHint, nil, nil)
	if err == nil {
...
func TestSizeHintWithCustomFlag(t *testing.T) {
	...
	desc, err := parser.buildTypeDescriptor(sliceType, nil, sizeHint, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestParserListWithFixedSizeRejected(t *testing.T) {
	...
	_, err := parser.buildTypeDescriptor(sliceType, typeHint, sizeHint, nil)
	if err == nil {
...
func TestParserListWithDynamicSizeAccepted(t *testing.T) {
	...
	desc, err := parser.buildTypeDescriptor(sliceType, typeHint, sizeHint, maxHint)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if desc.SszType != ssztypes.SszListType {
