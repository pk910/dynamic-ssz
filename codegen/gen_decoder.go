...
func (ctx *decoderContext) unmarshalContainer(desc *ssztypes.TypeDescriptor, varName string, typePath typePathList, indent int) error {
	...
	errCode := fmt.Sprintf("sszutils.ErrFixedFieldsEOFFn(dec.GetLength(), %s)", uintLitArg(totalStaticSizeExpr))
	ctx.appendCode(indent, "if %s {\n\treturn %s\n}\n", sizeCmpExpr(totalStaticSizeExpr, ">", lenExpr), typePath.getErrorWith(errCode))
	...
	for idx, fieldIdx := range dynamicFields {
		...
		// The trailing dynamic field runs to the end of the container. An open
		// region expresses that directly: it spans whatever remains of the
		// enclosing region, which equals maxOffset-offsetK when the length is
		// known and is the only correct choice when it is not.
		isTrailing := idx == len(dynamicFields)-1
		if isTrailing {
			ctx.appendCode(indent+1, "dec.PushOpenLimit()\n")
		} else {
			ctx.appendCode(indent+1, "dec.PushLimit(int(offset%d - offset%d))\n", dynamicFields[idx+1], fieldIdx)
		}
...
func (ctx *decoderContext) unmarshalVector(desc *ssztypes.TypeDescriptor, varName string, typePath typePathList, indent int, noBufCheck bool) error {
	...
	if desc.ElemDesc.SszTypeFlags&ssztypes.SszTypeFlagIsDynamic == 0 {
		...
		if desc.GoTypeFlags&ssztypes.GoTypeFlagIsByteArray != 0 {
			if !noBufCheck {
				errCode := fmt.Sprintf("sszutils.ErrByteVectorEOFFn(dec.GetLength(), %s)", uintLitArg(limitVar))
				ctx.appendCode(indent, "if %s {\n\treturn %s\n}\n", sizeCmpExpr(limitVar, ">", "dec.GetLength()"), typePath.getErrorWith(errCode))
			}
			expandSlice()
			if desc.GoTypeFlags&ssztypes.GoTypeFlagIsString != 0 {
		...
		if !noBufCheck {
			...
			errCode := fmt.Sprintf("sszutils.ErrVectorElementsEOFFn(dec.GetLength(), %s)", uintLitArg(sizeExpr))
			ctx.appendCode(indent, "if %s {\n\treturn %s\n}\n", sizeCmpExpr(sizeExpr, ">", "dec.GetLength()"), typePath.getErrorWith(errCode))
		}
		expandSlice()
		...
		errCode := fmt.Sprintf("sszutils.ErrVectorOffsetsEOFFn(dec.GetLength(), %s)", uintLitArg(tableSize))
		ctx.appendCode(indent, "if %s {\n\treturn %s\n}\n", sizeCmpExpr(tableSize, ">", "sszLen"), typePath.getErrorWith(errCode))
		startPosVar := fmt.Sprintf("startPos%d", ctx.startPosVarCounter)
		ctx.startPosVarCounter++
		ctx.appendCode(indent, "%s := dec.GetPosition()\n", startPosVar)
		...
		ctx.appendCode(indent, "if err != nil {\n\treturn %s\n}\n", typePath.getErrorWith("err"))
		errCode = fmt.Sprintf("sszutils.ErrFirstOffsetMismatchFn(firstOffset, %s)", uintLitArg(tableSize))
		ctx.appendCode(indent, "if %s {\n\treturn %s\n}\n", lenCmpExpr("int(firstOffset)", "!=", tableSize), typePath.getErrorWith(errCode))
		...
		// The declared length is a claim about the input, not a witness of it.
		// Seed the table from the bytes that have arrived and grow as the
		// offsets are read -- each costs four bytes.
		ctx.appendCode(indent+1, "offsetSlices[%d] = sszutils.ExpandSlice(offsetSlices[%d], sszutils.CredibleCount(dec, %s-1, 4))\n", ctx.offsetSliceCounter, ctx.offsetSliceCounter, intLitStr(limitVar))
		...
		ctx.appendCode(indent+2, "offsets = sszutils.GrowSlice(offsets, %s+1, %s-1)\n", indexVar, intLitStr(limitVar))
		ctx.appendCode(indent+2, "offsets[%s] = offset\n", indexVar)
		ctx.appendCode(indent+1, "}\n")
		ctx.appendCode(indent+1, "offsetSlices[%d] = offsets\n", ctx.offsetSliceCounter)
		ctx.appendCode(indent, "}\n")
		...
		// The trailing element runs to the end of the vector, so it takes an
...
func (ctx *decoderContext) unmarshalList(desc *ssztypes.TypeDescriptor, varName string, typePath typePathList, indent int) error {
	...
	if desc.ElemDesc.SszTypeFlags&ssztypes.SszTypeFlagIsDynamic == 0 {
		// static byte arrays
		if desc.GoTypeFlags&ssztypes.GoTypeFlagIsByteArray != 0 {
			// A byte list runs to the end of its region, so its length is only
			// knowable at EOF when the region is open. DecodeByteListInto covers
			// both regimes: it reuses the destination slice when the length is
			// known and reads to EOF otherwise, applying ssz-max as a read cap
		...
		ctx.appendCode(indent, "for %s := 0; itemCount < 0 || %s < itemCount; %s++ {\n", indexVar, indexVar, indexVar)
		// Open region: stop at end of input, and bound growth per element.
		ctx.appendCode(indent+1, "if itemCount < 0 {\n")
		ctx.appendCode(indent+2, "if more, err := dec.More(); err != nil {\n")
		ctx.appendCode(indent+3, "return %s\n", typePath.getErrorWith("err"))
...
func (ctx *decoderContext) unmarshalBitlist(desc *ssztypes.TypeDescriptor, varName string, typePath typePathList, indent int) error {
	...
	// A bitlist has no internal framing -- its terminator is the last byte of
	// the region -- so the payload always runs to the region end, which is only
	// discoverable at EOF when the region is open. The ssz-max bound is a byte
	// count either way (Limit/8+1 including the termination bit), so it doubles
...
func (ctx *decoderContext) unmarshalOptionalList(desc *ssztypes.TypeDescriptor, varName string, typePath typePathList, indent int) error {
	...
	ctx.appendCode(indent, "optEmpty, err := sszutils.RegionEmpty(dec)\n")
	ctx.appendCode(indent, "if err != nil {\n\treturn %s\n}\n", typePath.getErrorWith("err"))
	ctx.appendCode(indent, "if optEmpty {\n")
	ctx.appendCode(indent+1, "%s = nil\n", varName)
	ctx.appendCode(indent, "} else {\n")
	if desc.ElemDesc.SszTypeFlags&ssztypes.SszTypeFlagIsDynamic != 0 {
		errCodeEOF := "sszutils.ErrListOffsetsEOFFn(dec.GetLength(), 4)"
		ctx.appendCode(indent+1, "if dec.LengthKnown() && dec.GetLength() < 4 {\n\treturn %s\n}\n", typePath.getErrorWith(errCodeEOF))
...
func (ctx *decoderContext) unmarshalBigInt(desc *ssztypes.TypeDescriptor, varName string, typePath typePathList, indent int) error {
	...
	// doubles as a read cap so an over-long payload is never allocated.
