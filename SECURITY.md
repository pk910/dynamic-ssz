...
- **Major versions** are the only place where breaking changes may occur. Within the same major version, the public API of the root package, `treeproof`, `hasher` and `sszutils` and the behavior of generated code remain backward compatible.
- **Minor versions** introduce bigger features and new functionality.
- **Patch versions** contain bugfixes and small improvements.

The `reflection`, `ssztypes` and `codegen` packages and the `dynssz-gen` command are engine internals: their Go API and flags may change in a minor version. Code they generated stays covered by the guarantee above.
