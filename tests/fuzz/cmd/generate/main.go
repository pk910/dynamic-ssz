...
import (
	...
	"github.com/pk910/dynamic-ssz/tests/fuzz/typegen"
)
...
func findCorpusDir() string {
	...
	for {
		if _, err := os.Stat(filepath.Join(dir, "go.mod")); err == nil {
			return filepath.Join(dir, "tests", "fuzz", "corpus")
		}
...
func runCodegen(corpusDir string, typeNames []string, outputFile string, extended bool) error {
	// Find project root for dynssz-gen (corpusDir is tests/fuzz/corpus)
	projectRoot := filepath.Dir(filepath.Dir(filepath.Dir(corpusDir)))

	args := []string{
		"run", filepath.Join(projectRoot, "dynssz-gen"),
		"-package", "./tests/fuzz/corpus",
		"-package-name", "corpus",
...
func verifyCompilation(corpusDir string) error {
	...
	cmd := exec.Command("go", "build", "./tests/fuzz/corpus/...")
	cmd.Dir = projectRoot
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
